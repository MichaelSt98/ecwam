...
!
MODULE SDISSIP_CUF_HOIST_NEW_MOD
  CONTAINS
  ATTRIBUTES(DEVICE) SUBROUTINE SDISSIP_CUF_HOIST_NEW (KIJS, KIJL, FL1, FLD, SL, INDEP, WAVNUM, XK2CG, EMEAN, F1MEAN, XKMEAN,  &
  & UFRIC, COSWDIF, RAORW, CDIS, CDISVIS, CUMULW, DELTA_SDIS, G, INDICESSAT, IPHYS, IPSAT, MICHE, NANG, NDEPTH, NDIKCUMUL, NFRE,  &
  & NSDSNTH, RNU, SATWEIGHTS, SDSBR, SSDSC2, SSDSC3, SSDSC4, SSDSC5, SSDSC6, ZPI, ZPIFR, ICHNK, NCHNK, IJ)
    ...
    ! ----------------------------------------------------------------------
    USE SDISSIP_JAN_CUF_HOIST_NEW_MOD, ONLY: SDISSIP_JAN_CUF_HOIST_NEW
    USE SDISSIP_ARD_CUF_HOIST_NEW_MOD, ONLY: SDISSIP_ARD_CUF_HOIST_NEW
    USE cudafor
    ...
    SELECT CASE (IPHYS)
    CASE (0)
      CALL SDISSIP_JAN_CUF_HOIST_NEW(KIJS, KIJL, FL1(:, :, :, :), FLD(:, :, :), SL(:, :, :), WAVNUM(:, :, :), EMEAN(:),  &
      & F1MEAN(:), XKMEAN(:), CDIS, CDISVIS, DELTA_SDIS, NANG, NFRE, RNU, ZPI, ICHNK, NCHNK, IJ)
      
    CASE (1)
      CALL SDISSIP_ARD_CUF_HOIST_NEW(KIJS, KIJL, FL1(:, :, :, :), FLD(:, :, :), SL(:, :, :), INDEP(:, :), WAVNUM(:, :, :),  &
      & XK2CG(:, :, :), UFRIC(:, :), COSWDIF(:, :), RAORW(:), CUMULW(:, :, :, :), G, INDICESSAT(:, :), IPSAT, MICHE, NANG,  &
      & NDEPTH, NDIKCUMUL, NFRE, NSDSNTH, SATWEIGHTS(:, :), SDSBR, SSDSC2, SSDSC3, SSDSC4, SSDSC5, SSDSC6, ZPI, ZPIFR(:), ICHNK,  &
      & NCHNK, IJ)
    END SELECT
    
    
  END SUBROUTINE SDISSIP_CUF_HOIST_NEW
END MODULE SDISSIP_CUF_HOIST_NEW_MOD
