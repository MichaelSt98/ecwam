...
!
MODULE TRANSF_SNL_CUF_HOIST_NEW_MOD
  CONTAINS
  ATTRIBUTES(DEVICE) FUNCTION TRANSF_SNL_CUF_HOIST_NEW (XK0, D, XNU, SIG_TH, BATHYMAX, DKMAX, G, XKDMIN)
    ...
    REAL(KIND=JWRB) :: TRANSF_SNL_CUF_HOIST_NEW
...
!$acc routine seq
    ...
    IF (D < BATHYMAX .and. D > 0._JWRB) THEN
      X = XK0*D
      IF (X > DKMAX) THEN
        TRANSF_SNL_CUF_HOIST_NEW = 1._JWRB
      ELSE
        XK = MAX(XK0, XKDMIN / D)
        X = XK*D
        T_0 = TANH(X)
        ...
        TRANSF_SNL_CUF_HOIST_NEW = XNL**2 / (DV_G*T_0_SQ**4)
        TRANSF_SNL_CUF_HOIST_NEW = MAX(MIN(TRANSF_SNL_MAX, TRANSF_SNL_CUF_HOIST_NEW), TRANSF_SNL_MIN)
      END IF
    ELSE
      TRANSF_SNL_CUF_HOIST_NEW = 1._JWRB
    END IF
  ...
  END FUNCTION TRANSF_SNL_CUF_HOIST_NEW
END MODULE TRANSF_SNL_CUF_HOIST_NEW_MOD
