...
!
MODULE STRESS_GC_CUF_HOIST_NEW_MOD
  CONTAINS
  ATTRIBUTES(DEVICE) FUNCTION STRESS_GC_CUF_HOIST_NEW (ANG_GC, USTAR, Z0, Z0MIN, HALP, RNFAC, BETAMAXOXKAPPA2, BMAXOKAP,  &
  & C2OSQRTVG_GC, CM_GC, DELKCC_GC_NS, DELKCC_OMXKM3_GC, EPSUS, LLNORMAGAM, NWAV_GC, OM3GMKM_GC, OMXKM3_GC, RN1_RN, SQRTGOSURFT,  &
  & XKAPPA, XKMSQRTVGOC2_GC, XKM_GC, XK_GC, XLOGKRATIOM1_GC, ZALP)
    ...
    REAL(KIND=JWRB) :: STRESS_GC_CUF_HOIST_NEW
...
!$acc routine seq
    ...
    !     FIND NS:
    NS = NS_GC_CUF_HOIST_NEW(USTAR, NWAV_GC, SQRTGOSURFT, XKM_GC, XLOGKRATIOM1_GC)
    ...
    END DO
    STRESS_GC_CUF_HOIST_NEW = MAX(ZABHRC*TAUWCG, TAUWCG_MIN)
    
    
  END FUNCTION STRESS_GC_CUF_HOIST_NEW
END MODULE STRESS_GC_CUF_HOIST_NEW_MOD
