...
!
MODULE TRANSF_CUF_HOIST_NEW_MOD
  CONTAINS
  ATTRIBUTES(DEVICE) FUNCTION TRANSF_CUF_HOIST_NEW (XK, D, DKMAX, G)
    !
    ...
    REAL(KIND=JWRB) :: TRANSF_CUF_HOIST_NEW
...
!$acc routine seq
    ...
    IF (D < 999.0_JWRB .and. D > 0.0_JWRB) THEN
      ...
      ELSE
        ...
        XNL = XNL_1 - XNL_2
        TRANSF_CUF_HOIST_NEW = XNL**2 / (DV_G*T_0**8)
      END IF
    ELSE
      TRANSF_CUF_HOIST_NEW = 1.0_JWRB
    END IF
    !
  END FUNCTION TRANSF_CUF_HOIST_NEW
END MODULE TRANSF_CUF_HOIST_NEW_MOD
