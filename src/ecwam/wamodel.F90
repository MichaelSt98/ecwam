...
! CALL VARS_4D%F_FL1%GET_HOST_DATA_RDONLY(VARS_4D%XLLWS)
! !$acc exit data detach(VARS_4D%XLLWS)
...
! CALL MIJ%GET_HOST_DATA_RDONLY()
! CALL WVPRPT%GET_HOST_DATA_RDONLY()
...
! CALL INTFLDS%GET_HOST_DATA_RDONLY(WSEMEAN=.TRUE., WSFMEAN=.TRUE., USTOKES=.TRUE.,  &
!       & VSTOKES=.TRUE., STRNMS=.TRUE., TAUXD=.TRUE., TAUYD=.TRUE., TAUOCXD=.TRUE.,  &
