...
MODULE TAU_PHI_HF_FC_MOD
  ...
  ! - TAU_PHI_HF
  CONTAINS
  ATTRIBUTES(DEVICE) SUBROUTINE OMEGAGC_FC (UST, NS, XKS, OMS, NWAV_GC, OMEGA_GC, SQRTGOSURFT, XKM_GC, XK_GC, XLOGKRATIOM1_GC)
...
!$acc routine seq
    ...
    NS = NS_GC_FC(UST, NWAV_GC, SQRTGOSURFT, XKM_GC, XLOGKRATIOM1_GC)
    XKS = XK_GC(NS)
    OMS = OMEGA_GC(NS)
  ...
  ATTRIBUTES(DEVICE) SUBROUTINE TAU_PHI_HF_FC (KIJS, KIJL, MIJ, LTAUWSHELTER, UFRIC, Z0M, FL1, AIRD, RNFAC, COSWDIF, SINWDIF2,  &
  & UST, TAUHF, PHIHF, LLPHIHF, DELTH, FR5, G, GAMNCONST, GM1, JTOT_TAUHF, LLGCBZ0, LLNORMAGAM, NANG, NWAV_GC, OMEGA_GC,  &
  & SQRTGOSURFT, TAUWSHELTER, WTAUHF, X0TAUHF, XKAPPA, XKM_GC, XK_GC, XLOGKRATIOM1_GC, ZALP, ZPI4GM1, ZPI4GM2, ZPIFR, ICHNK,  &
  & NCHNK, IJ)
    ...
    IF (LLGCBZ0) THEN
      CALL OMEGAGC_FC(UFRIC(IJ, ICHNK), NS, XKS, OMS, NWAV_GC, OMEGA_GC(:), SQRTGOSURFT, XKM_GC(:), XK_GC(:), XLOGKRATIOM1_GC)
    END IF
