...
!
ATTRIBUTES(DEVICE) SUBROUTINE SNONLIN_FC (KIJS, KIJL, FL1, FLD, SL, WAVNUM, DEPTH, AKMEAN, AF11, BATHYMAX, COSTH, DAL1, DAL2,  &
& DELTH, DFIM, DFIMFR, DFIMFR2, DKMAX, FKLAM, FKLAM1, FKLAP, FKLAP1, FR, FRATIO, G, GM1, IKM, IKM1, IKP, IKP1, INLCOEF,  &
& ISNONLIN, K11W, K1W, K21W, K2W, KFRH, MFRSTLW, MLSTHG, NANG, NFRE, RNLCOEF, SINTH, TH, WETAIL, WP1TAIL, WP2TAIL, XKDMIN,  &
& ZPIFR, ICHNK, NCHNK, IJ, ENH, XNU, SIG_TH)
  ...
  CASE (1)
    ...
    DO MC=1,NFRE
      ENH(IJ, MC, ICHNK) = MAX(MIN(ENH_MAX, TRANSF_FC(WAVNUM(IJ, MC, ICHNK), DEPTH(IJ, ICHNK), DKMAX, G)), ENH_MIN)
    END DO
    DO MC=NFRE + 1,MLSTHG
      XK = GM1*(ZPIFR(NFRE)*FRATIO**(MC - NFRE))**2
      ENH(IJ, MC, ICHNK) = MAX(MIN(ENH_MAX, TRANSF_FC(XK, DEPTH(IJ, ICHNK), DKMAX, G)), ENH_MIN)
    END DO
    
    
  CASE (2)
    CALL PEAK_ANG_FC(KIJS, KIJL, FL1(:, :, :, :), XNU(:, ICHNK), SIG_TH(:, ICHNK), COSTH(:), DELTH, DFIM(:), DFIMFR(:),  &
    & DFIMFR2(:), FR(:), FRATIO, NANG, NFRE, SINTH(:), TH(:), WETAIL, WP1TAIL, WP2TAIL, ICHNK, NCHNK, IJ)
    
    DO MC=1,NFRE
      ENH(IJ, MC, ICHNK) =  &
      & TRANSF_SNL_FC(WAVNUM(IJ, MC, ICHNK), DEPTH(IJ, ICHNK), XNU(IJ, ICHNK), SIG_TH(IJ, ICHNK), BATHYMAX, DKMAX, G, XKDMIN)
    END DO
    DO MC=NFRE + 1,MLSTHG
      XK = GM1*(ZPIFR(NFRE)*FRATIO**(MC - NFRE))**2
      ENH(IJ, MC, ICHNK) = TRANSF_SNL_FC(XK, DEPTH(IJ, ICHNK), XNU(IJ, ICHNK), SIG_TH(IJ, ICHNK), BATHYMAX, DKMAX, G, XKDMIN)
    END DO
    ...
    IF (MC > MFR1STFR .and. MC < MFRLSTFR) THEN
      ...
      DO KH=1,2
        DO K=1,NANG
          ...
          !             ----------------------------------------------
          SAP =  &
          & GW1*FL1(IJ, K1, IP, ICHNK) + GW2*FL1(IJ, K11, IP, ICHNK) + GW3*FL1(IJ, K1, IP1, ICHNK) + GW4*FL1(IJ, K11, IP1, ICHNK)
          SAM =  &
          & GW5*FL1(IJ, K2, IM, ICHNK) + GW6*FL1(IJ, K21, IM, ICHNK) + GW7*FL1(IJ, K2, IM1, ICHNK) + GW8*FL1(IJ, K21, IM1, ICHNK)
