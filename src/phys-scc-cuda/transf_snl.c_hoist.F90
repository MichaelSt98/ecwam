...
!
ATTRIBUTES(DEVICE) FUNCTION TRANSF_SNL_FC (XK0, D, XNU, SIG_TH, BATHYMAX, DKMAX, G, XKDMIN)
...
!$acc routine seq
  ...
  IF (D < BATHYMAX .and. D > 0._JWRB) THEN
    ...
    ELSE
      ...
      TRANSF_SNL_FC = XNL**2 / (DV_G*T_0_SQ**4)
      TRANSF_SNL_FC = MAX(MIN(TRANSF_SNL_MAX, TRANSF_SNL_FC), TRANSF_SNL_MIN)
    END IF
  ELSE
    TRANSF_SNL_FC = 1._JWRB
  END IF
