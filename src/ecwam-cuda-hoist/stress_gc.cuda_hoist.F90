...
& XKMSQRTVGOC2_GC, XKM_GC, XK_GC, XLOGKRATIOM1_GC, ZALP, GAM_W) RESULT(STRESS_GC)
  ...
  REAL(KIND=JWRB), VALUE, INTENT(IN) :: ANG_GC  ! factor to account for angular spreading of the input.
  REAL(KIND=JWRB), VALUE, INTENT(IN) :: USTAR  ! friction velocity
  REAL(KIND=JWRB), VALUE, INTENT(IN) :: Z0  !  surface roughness
  REAL(KIND=JWRB), VALUE, INTENT(IN) :: Z0MIN  ! minimum surface roughness
  REAL(KIND=JWRB), VALUE, INTENT(IN) :: HALP  ! 1/2 Phillips parameter
  REAL(KIND=JWRB), VALUE, INTENT(IN) :: RNFAC  ! wind dependent factor used in the growth renormalisation
  ...
  REAL(KIND=JWRB), TARGET, INTENT(INOUT) :: GAM_W(:)
  
  !     INCLUDE FUNCTIONS FROM GRAVITY-CAPILLARY DISPERSION REALTIONS
  INTERFACE
    FUNCTION NS_GC_FC (USTAR) RESULT(NS_GC)
