MODULE FEMEANWS_FC_MOD
  ...
  & NCHNK, IJ, TEMP2, EM_LOC)
    ...
    INTERFACE
      ...
      & ICHNK, NCHNK, IJ, TEMP2, EM_LOC) BIND(c, name="femeanws_c_launch")
    ...
    REAL(KIND=JWRB), TARGET, INTENT(INOUT) :: EM_LOC(:, :)
!$acc host_data use_device( FL1, XLLWS, FM, EM, DFIM, DFIMOFR, FR, TEMP2, EM_LOC )
    CALL FEMEANWS_iso_c(KIJS, KIJL, c_loc(FL1), c_loc(XLLWS), c_loc(FM), c_loc(EM), DELTH, c_loc(DFIM), c_loc(DFIMOFR), EPSMIN,  &
    & c_loc(FR), FRTAIL, NANG, NFRE, WETAIL, ICHNK, NCHNK, IJ, c_loc(TEMP2), c_loc(EM_LOC))
!$acc end host_data
